...
namespace Kyub.GraphMaker
{
	...
	public static class WMG_Util
	{
		...
		public static Sprite createSprite(int x, int y)
		{
			...
			return Sprite.Create(newTex, new Rect(0, 0, x, y), new Vector2(0.5f, 0.5f), 100, 0, SpriteMeshType.FullRect);
		}
		...
		/// <summary>
		/// Whether or not compute shader can be used on this platform.
		/// </summary>
		/// <returns><c>true</c> if can use compute shader; otherwise, <c>false</c>.</returns>
		public static bool CanUseComputeShader()
		{
		...
		{
			Dictionary<string, float> inputVariables = new Dictionary<string, float>();
			inputVariables.Add("x", x);
			float result = ExpressionEvaluator(rpnString, inputVariables);
		...
		/// <summary>
		/// Evaluate arbitrary expression of the form (y = ...) for zero or more input variables, which can be defined via dictionary.
		/// Input expression must be in reverse polish notation with each token of the expression put in a List<string>
		/// </summary>
		/// <returns>The result is the output of the evaluated expression.</returns>
		/// <param name="rpnString">Reverse Polish Notation (RPN) string.</param>
		/// <param name="inputVariables">The input variables, defined by dictionary of names to values.</param>
		public static float ExpressionEvaluator(List<string> rpnString, Dictionary<string, float> inputVariables)
		{
			Stack<float> outputStack = new Stack<float>();
			for (int i = 0; i < rpnString.Count; i++)
			{
				if (IsOperator(rpnString[i]))
				{ // is an operator, pop values off stack, and evaluate those values using the operator
					if (OperatorIsUnary(rpnString[i]))
					{ // unary
				...
				{ // is a value, or the variable x, push it onto the stack
					float value = float.NaN;
					if (inputVariables.ContainsKey(rpnString[i]))
					{
						value = inputVariables[rpnString[i]];
		...
		public static List<string> GetNonMathematicalStringsFromEquationRPN(List<string> rpnString)
		{
			List<string> nonMathStrings = new List<string>();
			for (int i = 0; i < rpnString.Count; i++)
			{
				if (!IsOperator(rpnString[i]))
				{ // rpn string by definition has no parentheses, so just check if math operator
					double rpnStringNum = 0;
					bool stringIsNumeric = double.TryParse(rpnString[i], out rpnStringNum);
					if (!stringIsNumeric)
					{ // not math operator and not number
						nonMathStrings.Add(rpnString[i]);
					}
				}
			}
			return nonMathStrings;
		}

		private static string operatorsRegexStr = @" ?((\+)|(-)|(\*)|(/)|(\^)|(sqrt\()|(abs\()|(log\()|(ln\()|(sin\()|(cos\()|(tan\()) ?";
		private static string parenthesisRegexStr = @" ?((\()|(\))) ?";

		public static string GetFormattedEquationString(string mathematicalExpression)
		{
			Regex oneOrMoreWhitespaceRegex = new Regex(@"\s+");
			Regex operatorsRegex = new Regex(operatorsRegexStr);
			Regex parenthesesRegex = new Regex(parenthesisRegexStr);
			string formattedEquationStr = oneOrMoreWhitespaceRegex.Replace(mathematicalExpression, ""); // remove all white spaces
			formattedEquationStr = operatorsRegex.Replace(formattedEquationStr, @" $& "); // add a space between all mathematical operators and parentheses
			formattedEquationStr = parenthesesRegex.Replace(formattedEquationStr, @" $& ");
			formattedEquationStr = oneOrMoreWhitespaceRegex.Replace(formattedEquationStr, " "); // convert any double white spaces to single white space
			formattedEquationStr = formattedEquationStr.Trim(); // remove white spaces at the ends
			return formattedEquationStr;
		}
