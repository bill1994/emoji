	...
	public class WMG_Graph_Tooltip : WMG_GUI_Functions
	{
		...
		void Update()
		{
			if (theGraph.tooltipEnabled)
			{
				...
				{
					if (currentObj && !activeInHierarchy(currentObj))
					{ // otherwise if deactivate gameobject under mouse, then the mouse exit event never called
						MouseExitCommon();
		...
		void repositionTooltip()
		{
			...
			Vector3 position;
			RectTransformUtility.ScreenPointToWorldPointInRectangle(theGraph.toolTipPanel.transform as RectTransform,
				GetMousePosition(),
				(_canvas.renderMode == RenderMode.ScreenSpaceOverlay ? null : (_canvas.worldCamera == null ? Camera.main : _canvas.worldCamera)),
				out position);
		...
		void EnsureTooltipStaysOnScreen()
		{
			...
			ssCorners[0] = RectTransformUtility.WorldToScreenPoint(_canvas.renderMode == RenderMode.ScreenSpaceOverlay ? null : (_canvas.worldCamera == null ? Camera.main : _canvas.worldCamera), corners[0]);
			ssCorners[1] = RectTransformUtility.WorldToScreenPoint(_canvas.renderMode == RenderMode.ScreenSpaceOverlay ? null : (_canvas.worldCamera == null ? Camera.main : _canvas.worldCamera), corners[1]);
			ssCorners[2] = RectTransformUtility.WorldToScreenPoint(_canvas.renderMode == RenderMode.ScreenSpaceOverlay ? null : (_canvas.worldCamera == null ? Camera.main : _canvas.worldCamera), corners[2]);
			ssCorners[3] = RectTransformUtility.WorldToScreenPoint(_canvas.renderMode == RenderMode.ScreenSpaceOverlay ? null : (_canvas.worldCamera == null ? Camera.main : _canvas.worldCamera), corners[3]);
			...
			if (!Application.isEditor)
			{
				...
				{
					//targetDisplay = _canvas.targetDisplay;
				}
				else
				{
					targetDisplay = (_canvas.worldCamera == null ? Camera.main : _canvas.worldCamera).targetDisplay;
				}
			}
			Display disp = Display.displays[targetDisplay];

			if (maxX > disp.renderingWidth)
			{ // exceeded screen right edge
				offsetX = maxX - disp.renderingWidth;
			}
			else if (minX < 0)
			{ // exceeded screen left edge
				offsetX = minX;
			}

			if (maxY > disp.renderingHeight)
			{ // exceeded screen top edge
				offsetY = maxY - disp.renderingHeight;
			}
			else if (minY < 0)
			{ // exceeded screen bottom edge
				offsetY = minY;
			}
		...
		string defaultTooltipLabeler(WMG_Series aSeries, WMG_Node aNode)
		{
			...
			{
				if (aSeries.theGraph.useGroups)
				{
					textToSet = "(" + aSeries.theGraph.groups[Mathf.RoundToInt(nodeData.x - 1)] + ", " + nodeY + ")";
				}
				else
				{
			...
			if (aSeries.theGraph.tooltipDisplaySeriesName && !string.IsNullOrEmpty(aSeries.seriesName))
			{
		...
		{
			if (isTooltipObjectNull() || !aSeries.seriesTooltipEnabled) return;
			if (state)
			{
