...
namespace Kyub.GraphMaker
{
	...
	public class WMG_Events : WMG_GUI_Functions
	{
		...
		private void AddEventTrigger(UnityAction<GameObject, bool, PointerEventData> action, EventTriggerType triggerType, GameObject go, bool state)
		{
			...
			// Create a nee TriggerEvent and add a listener
			EventTrigger.TriggerEvent trigger = new EventTrigger.TriggerEvent();
			trigger.AddListener((eventData) => action(go, state, (PointerEventData)eventData)); // capture and pass the event data to the listener
																								// Create and initialise EventTrigger.Entry using the created TriggerEvent
			EventTrigger.Entry entry = new EventTrigger.Entry() { callback = trigger, eventID = triggerType };
			// Add the EventTrigger.Entry to delegates list on the EventTrigger
			eventTrigger.triggers.Add(entry);
		...
		#region MouseUpDownEvents

		// Ring graph
		public delegate void WMG_Ring_MouseDownUp_H(WMG_Ring ring, bool state, PointerEventData pointerEventData);
		/// <summary>
		/// Occurs when a mouse button does down or up on a ring graph ring / band. 
		/// @code
		/// graph.WMG_Ring_MouseDownUp += MyRingDownUp;
		/// void MyRingDownUp(WMG_Ring ring, bool down, UnityEngine.EventSystems.PointerEventData pointerEventData) {}
		/// @endcode
		/// </summary>
		public event WMG_Ring_MouseDownUp_H WMG_Ring_MouseDownUp;

		public void addRingMouseDownUpEvent(GameObject go)
		{
			AddEventTrigger(WMG_Ring_MouseDownUp_2, EventTriggerType.PointerDown, go, true);
			AddEventTrigger(WMG_Ring_MouseDownUp_2, EventTriggerType.PointerUp, go, false);
		}
