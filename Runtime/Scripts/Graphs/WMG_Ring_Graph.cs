...
namespace Kyub.GraphMaker
{
	...
	public class WMG_Ring_Graph : WMG_Graph_Manager
	{
		...
		{
			get { return _pieMode ? true : _bandMode; }
			set
			{
				if (_bandMode != value)
				{
		...
		public float pieModePaddingDegrees
		{
			get { return _pieModePaddingDegrees; }
			set
			{
		...
		public float pieModeDegreeOffset
		{
			get { return _pieModeDegreeOffset; }
			set
			{
		...
		{
			get { return _innerRadiusPercentage; }
			set
			{
		...
		{
			get { return _pieMode ? 0 : _degrees; }
			set
			{
				if (_degrees != value)
				{
		...
		{
			get { return _autoUpdateBandAlpha; }
			set
			{
		...
		public bool autoUpdateBandAlphaReverse
		{
			get { return _autoUpdateBandAlphaReverse; }
			set
			{
		...
		{
			get { return _pieMode ? 0 : _ringWidth; }
			set
			{
				if (_ringWidth != value)
				{
		...
		{
			get { return _ringPointWidthFactor; }
			set
			{
		...
		{
			get { return _pieMode ? 0 : _bandPadding; }
			set
			{
		...
		/// <summary>
		/// Whether to hide all labels including the label lines and label line points.
		/// </summary>
		/// <value><c>true</c> if hide all labels; otherwise, <c>false</c>.</value>
		public bool hideAllLabels
		{
			get { return _hideAllLabels; }
			set
			{
				if (_hideAllLabels != value)
				{
					_hideAllLabels = value;
					labelsVisbilityC.Changed();
				}
			}
		}
		/// <summary>
		...
		public bool hideZeroLabelLine
		{
			get { return _hideZeroLabelLine; }
			set
			{
		...
		public bool labelStartCenteredOnBand
		{
			get { return _labelStartCenteredOnBand; }
			set
			{
		...
		/// <summary>
		/// Gets or sets the width of the label line.
		/// </summary>
		/// <value>The width of the label line.</value>
		public float labelLineWidth
		{
			get { return _labelLineWidth; }
			set
			{
		...
		public Vector2 labelPointSize
		{
			get { return _labelPointSize; }
			set
			{
		...
		public float labelTextOffset
		{
			get { return _labelTextOffset; }
			set
			{
		...
		/// <summary>
		/// Gets or sets the label text vertical offset.
		/// </summary>
		/// <value>The label text vertical offset.</value>
		public float labelTextVerticalOffset
		{
			get { return _labelTextVerticalOffset; }
			set
			{
				if (_labelTextVerticalOffset != value)
				{
					_labelTextVerticalOffset = value;
					labelLineC.Changed();
				}
			}
		}
		...
		public Color labelLineColor
		{
			get { return _labelLineColor; }
			set
			{
		...
		public Color labelPointColor
		{
			get { return _labelPointColor; }
			set
			{
	...
	public Font labelsFont { get {return _labelsFont;} 
		...
		{
			get { return _dataLabelsFontSize; }
			set
			{
		...
		{
			get { return _dataLabelsFontStyle; }
			set
			{
...
#if TMP_PRESENT
		public TMP_FontAsset dataLabelsFont
		{
			get { return _dataLabelsFont; }
			set
			{
	...
	public Font dataLabelsFont { get {return _dataLabelsFont;} 
		...
		{
			get { return _antiAliasingStrength; }
			set
			{
		...
		[SerializeField] private bool _hideAllLabels;
		...
		[SerializeField] private float _labelTextVerticalOffset = 1;
...
#if TMP_PRESENT
		[SerializeField] private TMP_FontAsset _labelsFont;
#else
	[SerializeField] private Font _labelsFont; 
#endif
		[SerializeField] private float _labelLineWidth = 2;
	...
	[SerializeField] private Font _dataLabelsFont;  
		...
		public float RingWidthFactor { get { return origGraphWidth == 0 ? 0 : (((1 - innerRadiusPercentage) * outerRadius) / origGraphWidth); } }
		...
		// private with change tracking
		private bool isEmptyGraph
		{
			get { return _isEmptyGraph; }
			set
			{
				if (_isEmptyGraph != value)
				{
					_isEmptyGraph = value;
					labelsVisbilityC.Changed();
				}
		...
		private WMG_Change_Obj labelsVisbilityC = new WMG_Change_Obj();
		private WMG_Change_Obj labelLineWidthC = new WMG_Change_Obj();
		...
		public delegate string DataLabeler(WMG_Ring_Graph graph, float val, int index);
		public DataLabeler dataLabeler;
		public string defaultDataLabeler(WMG_Ring_Graph graph, float val, int index)
		{
		...
		public void Init()
		{
			...
			changeObjs.Add(labelsVisbilityC);
			changeObjs.Add(labelLineWidthC);
			...
			labelsVisbilityC.OnChange += LabelsVisibilityChanged;
			labelLineWidthC.OnChange += updateLabelLineWidths;
		...
		void DegreesChanged()
		{
			...
			{ // pie mode or count changed more than once in a frame, update all
		...
		{
			if (pieMode || aRingValChangeIndices.Count > 1)
			{ // pie mode or multiple vals changed in a frame, update all
		...
		public void valuesChanged(bool editorChange, bool countChanged, bool oneValChanged, int index)
		{
			...
			labelLineC.Changed(); // otherwise text width set after, label line position set which uses text width
		}
		...
		void updateSizesOfAllObjects()
		{
			...
			for (int i = 0; i < rings.Count; i++)
			{
				...
				changeSpriteHeightFloat(rings[i].label, labelLineStartOffset + labelLinePadding);
			}
			// zero line
			changeSpriteHeightFloat(zeroLine, outerRadius + labelLinePadding);
			// label line points
			for (int i = 0; i < rings.Count; i++)
			{
				if (!pieMode)
				{
					...
					// label points
					if (bandMode && labelStartCenteredOnBand)
					{ // center on bands
		...
		{
			setCanvasGroupAlpha(ringLabelsParent, (hideAllLabels || isEmptyGraph) ? 0 : 1);
			for (int i = 0; i < rings.Count; i++)
			{
		...
		void updateLabelLineWidths()
		{
			for (int i = 0; i < rings.Count; i++)
			{
				changeSpriteWidthFloat(rings[i].label, labelLineWidth);
			}
		}
		...
		void updateLabelsText()
		{
			for (int i = 0; i < rings.Count; i++)
			{
				...
				Vector2 textSize = getSpriteSize(rings[i].labelText);
				changeSpriteSizeFloat(rings[i].labelBackground, textSize.x, textSize.y);
				...
				if (showDataLabels)
				{
					...
					// Set Text
					changeLabelText(rings[i].lowerLabelText, dataLabeler(this, values[i], i));

					Vector2 lowerTextSize = getSpriteSize(rings[i].lowerLabelText);
					changeSpriteSizeFloat(rings[i].lowerLabelBackground, lowerTextSize.x, lowerTextSize.y);

					changeSpriteHeightFloat(rings[i].textLine, Mathf.Max(textSize.x, lowerTextSize.x) + labelTextOffset);
				}
				else
				{
					changeSpriteHeightFloat(rings[i].textLine, textSize.x + labelTextOffset);
				}
			}
		}
		...
		Vector3 getRingBaseRotation(float previousCumulativeValue, float totalVal)
		{
			if (pieMode && totalVal == 0)
			{
				return Vector3.zero;
			}
			return new Vector3(0, 0, pieMode ? (-previousCumulativeValue / totalVal * 360f - pieModeDegreeOffset) : -degrees / 2);
		}

		float getRingValPercentage(float val, float totalVal)
		{
			if (pieMode && totalVal == 0)
			{
				isEmptyGraph = true;
			...
			if (!pieMode && maxValue == minValue)
			{
				isEmptyGraph = true;
				return 0;
			}
			isEmptyGraph = false;
			return pieMode ? (val / totalVal - 2 * pieModePaddingDegrees / 360f) : (val / (maxValue - minValue));
		}
		...
		void updateDegreesAring(int i, bool countChanged, float totalVal, float previousCumulativeValue)
		{
			...
			Vector3 baseRotation = getRingBaseRotation(previousCumulativeValue, totalVal);
			float valPercent = getRingValPercentage(values[i], totalVal);
			...
			{
				if (zeroLine.activeSelf)
				{ // previously not 0
					SetActive(zeroLine, !ringIsZero);
				}
				else
				{ // there was a zero at one point in the past, always hide
		...
		void updateLabelLineAring(int i, bool countChanged, float totalVal, Vector3 baseRotation, float valPercent)
		{
			...
			if (!pieMode)
			{
				for (int j = i - 1; j >= 0; j--)
				{
					float valPercentPrev = getRingValPercentage(values[j], totalVal);
					if (Mathf.Abs(valPercent - valPercentPrev) < 0.01f)
					{ // within 1%
						numOverlapping++;
			...
			if (animateData)
			{
				if (DOTween.IsTweening(rings[i].label.transform) || !countChanged)
				{ // if already animating, then don't animate relative to current rotation
		...
		{
			changeSpriteWidthFloat(ring.textLine, labelLineWidth + (getSpriteHeight(ring.labelBackground) + labelLineWidth + labelTextVerticalOffset) * numOverlapping);
		...
		void updateLabelRotationAndPosition(WMG_Ring ring, float degOffset, int numOverlapping, bool onComplete)
		{
			...
			if (labelOnRightSide)
			{
				...
				setAnchor(ring.labelBackground, Vector2.one, new Vector2(1, 0), new Vector2(-labelLineWidth - labelTextVerticalOffset, 0));
				setAnchor(ring.lowerLabelBackground, Vector2.one, Vector2.zero, new Vector2(getSpriteHeight(ring.lowerLabelBackground) + labelTextVerticalOffset, -getSpriteWidth(ring.lowerLabelBackground)));
			}
			else
			{
				if (labelsOverlap)
				...
				setAnchor(ring.labelBackground, new Vector2(0, 1), Vector2.zero, new Vector2(labelLineWidth + labelTextVerticalOffset, 0));
				setAnchor(ring.lowerLabelBackground, new Vector2(0, 1), Vector2.zero, new Vector2(-getSpriteHeight(ring.lowerLabelBackground) - labelTextVerticalOffset, 0));
			}
		}
		...
		void labelRotationComplete(WMG_Ring ring, float degOffset, int numOverlapping)
		{
			bool labelsOverlap = numOverlapping > 0;
			if (labelsOverlap)
			{ // if label is overlapping only update at the end
				updateLabelLineBasedOnOverlap(ring, numOverlapping);
