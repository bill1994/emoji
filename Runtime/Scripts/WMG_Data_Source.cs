...
namespace Kyub.GraphMaker
{
	...
	public class WMG_Data_Source : MonoBehaviour
	{
		...
		*/ // DELETE THIS LINE FOR USE WITH PLAYMAKER
		...
		void parseStrings()
		{ // variable name(s) can be of the format property.field, parse these.
			varNames1.Clear();
			varNames2.Clear();
		...
		public List<T> getData<T>()
		{
			...
			{
				try
				{ // try field, e.g. public List<Vector2> test;
		...
		T getDatum<T>(object dp, string variableName, WMG_VariableTypes varType, string vName1, string vName2)
		{
			...
			{
				try
				{ // try field, e.g. public float test;
			...
			{
				try
				{ // try property, e.g. public float Test {get; private set};
			...
			{
				try
				{ // try property.field, e.g. localPosition.y from a Transform dataProvider
			...
			{
				try
				{ // try field.field, e.g. test.y from public Vector3 test;
