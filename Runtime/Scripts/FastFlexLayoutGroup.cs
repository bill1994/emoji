    ...
    public class FastFlexLayoutGroup : FastHorizontalOrVerticalLayoutGroup
    {
        ...
        protected virtual List<LinearGroup> BuildChildrenGroups(bool isVertical)
        {
            ...
            for (int i = 0; i < children.Count; i++)
            {
                ...
                if (pos + childSize <= (startPos + innerSize) && (m_GroupCapacity <= 0 || currentGroup.Children.Count < m_GroupCapacity))
                {
        ...
        protected void SetChildrenAlongGroupAxis(int axis, bool isVertical, LinearGroup group)
        {
            ...
            else
            {
                ...
                var remainingSurplusPerChilden = GetRemainingSurplusPerChildren(group.AxisPreFilterIndexes, group.Children, axis, controlSize, childForceExpandSize, minMaxLerp, itemFlexibleMultiplier, totalFlexibleSize);
