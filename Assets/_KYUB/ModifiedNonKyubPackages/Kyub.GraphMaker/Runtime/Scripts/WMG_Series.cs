...
namespace Kyub.GraphMaker
{
	...
	public class WMG_Series : MonoBehaviour
	{
		...
		{
			get { return _UseXDistBetweenToSpace; }
			set
			{
		...
		{
			get { return _ManuallySetXDistBetween; }
			set
			{
		...
		{
			get { return _xDistBetweenPoints; }
			set
			{
		...
		{
			get { return _ManuallySetExtraXSpace; }
			set
			{
		...
		{
			get { return _connectFirstToLast; }
			set
			{
		...
		{
			get { return _dataLabelsNumDecimals; }
			set
			{
	...
	public Font dataLabelsFont { get {return _dataLabelsFont;} 
		...
		public bool dataLabelsAnchoredLeftBot
		{
			get { return _dataLabelsAnchoredLeftBot; }
			set
			{
		...
		public bool linesUseComputeShader
		{
			get { return _linesUseComputeShader; }
			set
			{
		...
		{
			get { return _areaShadingUsesComputeShader; }
			set
			{
		...
		{
			get { return _areaShadingAxisValue; }
			set
			{
	...
	[SerializeField] private Font _dataLabelsFont;
		...
		public string formatSeriesDataLabel(WMG_Series series, float val, int labelIndex)
		{
			...
			{ // graph uses groups and series x point is negative always means NULL
				return "";
			}
		...
		void PointValuesValChanged()
		{
			...
			{ // multiple single vals changed in a single frame
				theGraph.aSeriesPointsChanged();
				pointValuesChanged();
			}
			else
		...
		void createOrDeletePoints(int pointValuesCount)
		{
			...
			// Create line connecting first and last points
			if (!neverCreateLines)
			{
				if (points.Count > 2)
				{
		...
		void UpdateSprites(int index = -1)
		{
			...
			if (beginningToAutoAnimate)
			{
				...
				{ // not already in the middle of another animation
			...
			if (beginningToAutoAnimate)
			{ // if beginning auto animation, trigger the animation to begin
			  // update new positions / animation end positions
				getNewPointPositionsAndSizes(prevPoints, -1); // single point animation tricky, because animation can be interrupted by another animation in a later frame, and the position lists would not be correct
		...
		void getNewPointPositionsAndSizes(List<GameObject> prevPoints, int index)
		{
			...
			// cache various values before looping through points - performance optimization
			int pointValuesCount = pointValues.Count;
			...
			for (int i = 0; i < pointValuesCount; i++)
			{
				...
				{
					if (i > 0)
					{ // For points greater than 0, use the previous point position plus xDistBetween
						float prevPosX = (isVertical) ? afterPositions[i - 1].x : afterPositions[i - 1].y;
						newX = prevPosX + xDistBetweenPoints;
					}
					else
					{ // For point 0, set initial x position to extraXSpace
						newX = extraXSpace - (!_seriesIsLine && !isVertical ? theGraphBarWidth : 0);
					}
				}
				else if (theGraphUseGroups)
				{ // Using groups, x values represent integer index of group
					newX = extraXSpace + xDistBetweenPoints * (Mathf.Abs(pointValues[i].x) - 1);
					newX -= (!_seriesIsLine && !isVertical ? theGraphBarWidth : 0);
				}
				else
				{ // Not using xDistBetween or groups, so use the actual x values in the Vector2 list
		...
		void updateAreaShading()
		{
			...
			if ((areaShadingUsesComputeShader || linesUseComputeShader) && areaShadingRects.Count == 1)
			{ // all values (min, max, and the points) represent a percentage of graph
				int pointValuesCount = pointValues.Count;
				if (pointValuesCount < 2)
				...
				float maxVal = 0; // as a percentage of axis
				...
				{
					if (neverCreatePoints)
					{ // automatic animations dont work here
					...
					if (graphUsesGroups && pointValues[i].x < 0)
					{ // null data point
						areaShadingCSdata.xVals[i] = float.NaN;
						areaShadingCSdata.yVals[i] = float.NaN;
					}
					if (!graphUsesGroups && float.IsNaN(pointValues[i].y))
					{// null data point
						areaShadingCSdata.xVals[i] = float.NaN;
						areaShadingCSdata.yVals[i] = float.NaN;
					}
			...
			else
			{
				...
				for (int i = 0; i < pointsCount - 1; i++)
				{
					...
					if (i > 0)
					{ // Don't need to adjust the width for the first rectangle
						if (theGraph.orientationType == WMG_Axis_Graph.orientationTypes.horizontal)
						{
		...
		void DoRealTimeUpdate()
		{
			...
			if (pointValues.Count >= 2)
			{
				...
				if (Mathf.Abs(slopes[0] - slopes[1]) <= maxStdDev)
				{ // if standard deviation less than threshold, then move point
