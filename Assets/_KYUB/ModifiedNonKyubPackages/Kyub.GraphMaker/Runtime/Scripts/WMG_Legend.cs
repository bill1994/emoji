...
namespace Kyub.GraphMaker
{
	...
	public class WMG_Legend : WMG_GUI_Functions
	{
		...
		{
			get { return _oppositeSideLegend; }
			set
			{
		...
		{
			get { return _setWidthFromLabels; }
			set
			{
		...
		{
			get { return _legendEntryLinkSpacing; }
			set
			{
		...
		{
			get { return _legendEntryFontSize; }
			set
			{
		...
		{
			get { return _legendEntrySpacing; }
			set
			{
		...
		{
			get { return _legendEntryFontStyle; }
			set
			{
...
#if TMP_PRESENT
		public TMP_FontAsset legendEntryFont
		{
			get { return _legendEntryFont; }
			set
			{
...
#if TMP_PRESENT
		[SerializeField] private TMP_FontAsset _legendEntryFont;
#else
	[SerializeField] private Font _legendEntryFont;
		...
		float getMaxLabelWidth()
		{
			...
			{
				float labelWidth = getSpriteSize(entry.label).x * entry.label.transform.localScale.x;
		...
		public void updateLegend()
		{
			...
			for (int j = 0; j < legendEntries.Count; j++)
			{
				...
				{
					changeSpriteWidthFloat(legendEntry.swatchNode, pieSwatchSize);
					changeSpriteHeightFloat(legendEntry.swatchNode, pieSwatchSize);
				}
				...
				if (legendType == legendTypes.Bottom)
				{
					...
					else
					{
						if (numRowsOrColumns > 1)
						{
							...
							if (LegendWidth > getSpriteWidth(theGraph.gameObject))
							{ // if new temporary width exceeds border, then dont do anything
								_numRowsOrColumns++;
							}
							else
							{ // new width doesn't exceed the border, actually decrease
								_numRowsOrColumns++;
								numRowsOrColumns--;
							}
						}
