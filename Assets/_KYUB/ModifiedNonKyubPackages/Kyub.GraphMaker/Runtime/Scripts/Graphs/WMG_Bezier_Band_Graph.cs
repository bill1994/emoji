...
namespace Kyub.GraphMaker
{
	...
	public class WMG_Bezier_Band_Graph : WMG_Graph_Manager
	{
		...
		{
			get { return _startHeightPercent; }
			set
			{
		...
		void UpdateFontSize()
		{
			for (int i = 0; i < values.Count; i++)
			{
				...
				refreshTextRectTransformSize(bands[i].percentLabel);
				refreshTextRectTransformSize(bands[i].label);
			}
		}
