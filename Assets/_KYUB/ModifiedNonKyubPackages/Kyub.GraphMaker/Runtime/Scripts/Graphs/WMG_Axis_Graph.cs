...
namespace Kyub.GraphMaker
{
	...
	public class WMG_Axis_Graph : WMG_Graph_Manager
	{
		...
		public enum axesTypes { MANUAL, CENTER, AUTO_ORIGIN, AUTO_ORIGIN_X, AUTO_ORIGIN_Y, I, II, III, IV, I_II, III_IV, II_III, I_IV, DUAL_Y, DUAL_Y_MID, DUAL_Y_TOP };
		[System.Flags]
		...
		public orientationTypes orientationType
		{
			...
			set
			{
				...
				{
					if (IsDualY && value == orientationTypes.horizontal)
					{
		...
		{
			get { return _axesType; }
			set
			{
				if (_axesType != value)
				{
					if (orientationType == orientationTypes.horizontal && (value == axesTypes.DUAL_Y || value == axesTypes.DUAL_Y_MID || value == axesTypes.DUAL_Y_TOP))
					{
						Debug.LogWarning("Cannot change axes to dual-y for a horizontally oriented chart");
						return;
					}
					if (yAxis2 == null && (value == axesTypes.DUAL_Y || value == axesTypes.DUAL_Y_MID || value == axesTypes.DUAL_Y_TOP))
					{
		...
		{
			get { return _autoPaddingEnabled; }
			set
			{
		...
		public AutoPaddingProperties autoPaddingProperties
		{
			get { return _autoPaddingProperties; }
			set
			{
		...
		{
			get { return _autoUpdateBarWidth; }
			set
			{
		...
		{
			get { return _autoUpdateBarWidthSpacing; }
			set
			{
				if (_autoUpdateBarWidthSpacing != value)
				{
					_autoUpdateBarWidthSpacing = Mathf.Clamp01(value);
					graphC.Changed();
					seriesNoCountC.Changed();
				}
			}
		...
		{
			get { return _autoUpdateSeriesAxisSpacing; }
			set
			{
		...
		{
			get { return _autoUpdateBarAxisValue; }
			set
			{
		...
		{
			get { return _autoShrinkAtPercent; }
			set
			{
		...
		{
			get { return _autoGrowAndShrinkByPercent; }
			set
			{
		...
		{
			get { return _autoAnimationsEnabled; }
			set
			{
		...
		public int graphTitleSize
		{
			get { return _graphTitleSize; }
			set
			{
		...
		public bool IsDualY
		{
			get
			{
				return (axesType == WMG_Axis_Graph.axesTypes.DUAL_Y
						|| axesType == WMG_Axis_Graph.axesTypes.DUAL_Y_MID
						|| axesType == WMG_Axis_Graph.axesTypes.DUAL_Y_TOP);
			}
		}
		...
		public WMG_Change_Obj stackedSeriesCountC = new WMG_Change_Obj();
		public WMG_Change_Obj stackedSeriesNoCountC = new WMG_Change_Obj();
		...
		public delegate void BarWidthUpdater(WMG_Axis_Graph graph);
		/// <summary>
		/// Use to override the default update function for #autoUpdateBarWidth.
		/// @code
		/// graph.barWidthUpdater = customBarWidthUpdater;
		/// void customBarWidthUpdater(WMG_Axis_Graph graph) {}
		/// @endcode
		/// </summary>
		public BarWidthUpdater barWidthUpdater;
		...
		public virtual void Init()
		{
			...
			changeObjs.Add(stackedSeriesCountC);
			changeObjs.Add(stackedSeriesNoCountC);
			...
			barWidthUpdater = UpdateBarWidth;
			...
			stackedSeriesCountC.OnChange += StackedSeriesCountChanged;
			stackedSeriesNoCountC.OnChange += StackedSeriesNoCountChanged;
		...
		void GraphTypeChanged()
		{
			...
			for (int j = 0; j < lineSeries.Count; j++)
			{
				if (!activeInHierarchy(lineSeries[j])) continue;
				WMG_Series theSeries = lineSeries[j].GetComponent<WMG_Series>();
				theSeries.pointValuesCountChangedSeries(true);
			}
			EnsureSeriesOrderedCorrectly();
		}

		void EnsureSeriesOrderedCorrectly()
		{
			...
			if (graphType == graphTypes.line_stacked)
			{
				for (int j = 0; j < lineSeries.Count; j++)
				{
					lineSeries[j].transform.SetAsFirstSibling();
				}
			}
		}
		...
		void SeriesChanged(bool countChanged, bool instant)
		{
			for (int j = 0; j < lineSeries.Count; j++)
			{
				...
				if (countChanged)
				{
					...
					{
						theSeries.pointValuesCountCChanged();
					}
				}
				else
				{
		...
		void StackedSeriesCountChanged()
		{
			aSeriesPointsChanged();
			SeriesChanged(true, true);
		}

		void StackedSeriesNoCountChanged()
		{
			aSeriesPointsChanged();
			SeriesChanged(false, true);
		}
		...
		void UpdateAxesType()
		{
			...
			if (yAxis2 != null)
			{
				yAxis2.AxisTitle.SetActive(IsDualY);
				yAxis2.AxisTicks.SetActive(IsDualY);
				yAxis2.AxisLine.SetActive(IsDualY);
				yAxis2.AxisLabelObjs.SetActive(IsDualY);
			}
			...
			else
			{
				...
				if (axesType == axesTypes.I || axesType == axesTypes.II || axesType == axesTypes.III || axesType == axesTypes.IV
					|| axesType == axesTypes.DUAL_Y || axesType == axesTypes.DUAL_Y_TOP)
				{
					...
					else if (axesType == axesTypes.DUAL_Y_TOP)
					{
						setAxesDualYaxisTop();
					}
				}
				else
				{
					...
					else if (axesType == axesTypes.DUAL_Y_MID)
					{
						setAxesDualYaxisMid();
						yAxis2.possiblyHideTickBasedOnPercent();
					}
		...
		void updateOriginRelativeToAxes()
		{
			if (autoUpdateOrigin)
			{
				...
				else if (axesType == axesTypes.IV || axesType == axesTypes.DUAL_Y_TOP)
				{
				...
				else if (axesType == axesTypes.I_IV || axesType == axesTypes.DUAL_Y_MID)
				{
		...
		void AutoPadding()
		{
			if (autoPaddingEnabled)
			{
				...
				// temporarily set rotation to 0 relative to canvas, so calculations are simple
				Canvas thisCanvas = GetComponentInParent<Canvas>();
				Quaternion originalRot = this.transform.rotation;
				this.transform.rotation = thisCanvas.transform.rotation;
				...
				{
					_paddingLeftRight = new Vector2(Mathf.Max(paddingLeftRight.x - xDif.x, 0), Mathf.Max(paddingLeftRight.y - xDif.y, 0));
				}
				if (Mathf.Abs(yDif.x) > 0.1 || Mathf.Abs(yDif.y) > 0.1)
				{
					_paddingTopBottom = new Vector2(Mathf.Max(paddingTopBottom.x - yDif.y, 0), Mathf.Max(paddingTopBottom.y - yDif.x, 0));
				}
		...
		void getAutoPaddingDifferentialFromGraphContent(ref Vector2 xDif, ref Vector2 yDif)
		{
			...
			if (IsDualY && activeInHierarchy(yAxis2.AxisArrowDL))
			{
				getRectDiffs(yAxis2.AxisArrowDL, this.gameObject, ref xDif, ref yDif);
				getMinimumBorderDiffs(ref dLeft, ref dRight, ref dBot, ref dTop, xDif, yDif);
			}
			if (IsDualY && activeInHierarchy(yAxis2.AxisArrowUR))
			{
			...
			if (IsDualY && activeInHierarchy(yAxis2.AxisLine) && getSpriteAlpha(yAxis2.AxisLine) > 0)
			{
			...
			if ((autoPaddingProperties & AutoPaddingProperties.YaxisTitle) == AutoPaddingProperties.YaxisTitle)
			{
				...
				{
					getRectDiffs(yAxis.AxisTitle, this.gameObject, ref xDif, ref yDif);
					getMinimumBorderDiffs(ref dLeft, ref dRight, ref dBot, ref dTop, xDif, yDif);
				}
			}
			if ((autoPaddingProperties & AutoPaddingProperties.Y2axisTitle) == AutoPaddingProperties.Y2axisTitle)
			{
				if (IsDualY && activeInHierarchy(yAxis2.AxisTitle) && !string.IsNullOrEmpty(yAxis2.AxisTitleString))
				{
					getRectDiffs(yAxis2.AxisTitle, this.gameObject, ref xDif, ref yDif);
					getMinimumBorderDiffs(ref dLeft, ref dRight, ref dBot, ref dTop, xDif, yDif);
				}
			}
			if ((autoPaddingProperties & AutoPaddingProperties.XaxisTitle) == AutoPaddingProperties.XaxisTitle)
			{
				...
				{
					getRectDiffs(xAxis.AxisTitle, this.gameObject, ref xDif, ref yDif);
					getMinimumBorderDiffs(ref dLeft, ref dRight, ref dBot, ref dTop, xDif, yDif);
				}
			}
			if ((autoPaddingProperties & AutoPaddingProperties.Title) == AutoPaddingProperties.Title)
			{
				...
				{
					getRectDiffs(graphTitle, this.gameObject, ref xDif, ref yDif);
					getMinimumBorderDiffs(ref dLeft, ref dRight, ref dBot, ref dTop, xDif, yDif);
				}
			}
		...
		void UpdateSeriesParentPositions()
		{
			...
			Vector3 newChildPos = new Vector3(-cachedContainerWidth * 0.5f - (paddingLeftRight.x - paddingLeftRight.y) / 2f + paddingLeftRight.x, // + (paddingLeftRight.x - paddingLeftRight.y)/2f
											  -cachedContainerHeight * 0.5f - (paddingTopBottom.y - paddingTopBottom.x) / 2f + paddingTopBottom.y); // + (paddingTopBottom.y - paddingTopBottom.x)/2f
			...
			for (int j = 0; j < lineSeriesCount; j++)
			{
				...
				if (theSeries.seriesIsLine)
				{
					...
					{
						changeSpritePositionTo(lineSeries[j], newChildPos + new Vector3(theSeries.extraXSpace, 0, 0));
						continue;
					}
					changeSpritePositionTo(lineSeries[j], newChildPos);
				}
				else
				{
					if (orientationType == orientationTypes.vertical)
					{
						if (theSeries.ManuallySetExtraXSpace)
						{
							changeSpritePositionTo(lineSeries[j], newChildPos + new Vector3(theSeries.extraXSpace, axisWidthOffset.y, 0));
							continue;
						}
						changeSpritePositionTo(lineSeries[j], newChildPos + new Vector3(axisWidthOffset.x, axisWidthOffset.y, 0));
					}
					else
					{
						if (theSeries.ManuallySetExtraXSpace)
						{
							changeSpritePositionTo(lineSeries[j], newChildPos + new Vector3(theSeries.extraXSpace, axisWidthOffset.y + barWidth, 0));
							continue;
						}
						changeSpritePositionTo(lineSeries[j], newChildPos + new Vector3(axisWidthOffset.x, axisWidthOffset.y + barWidth, 0));
					}
				}
		...
		public void InEditorUpdate()
		{
			if (Application.isEditor)
			{
				...
				changeSpriteSizeFloat(graphBackground, newSize.x, newSize.y);
		...
		void UpdateBG()
		{
			...
			for (int j = 0; j < lineSeriesCount; j++)
			{
				if (!activeInHierarchy(lineSeries[j])) continue;
				WMG_Series theSeries = lineSeries[j].GetComponent<WMG_Series>();
				if (theSeries.areaShadingUsesComputeShader || theSeries.linesUseComputeShader)
				{
					if (theSeries.areaShadingCSPrefab == null || theSeries.areaShadingParent == null) continue;
					if ((theSeries.areaShadingType != WMG_Series.areaShadingTypes.None || theSeries.linesUseComputeShader) && theSeries.getAreaShadingRects().Count == 1)
					{
		...
		{
			changeSpritePositionTo(graphBackground.transform.parent.gameObject, new Vector3(-x * (0.5f) + paddingLeftRight.x, -y * (0.5f) + paddingTopBottom.y));
			changeSpritePositionTo(seriesParent, new Vector3(-x * (0.5f) + paddingLeftRight.x + xAxisLength / 2f, -y * (0.5f) + paddingTopBottom.y + yAxisLength / 2f));
			changeSpriteSizeFloat(seriesParent, xAxisLength, yAxisLength);
		}
		...
		void UpdateTotals()
		{
			...
			// Cache series list info - this is an optimization to reduce function call overhead
			int numSeries = 0;
			List<WMG_Series> theSeries = new List<WMG_Series>();
			List<int> theSeriesCount = new List<int>();
			List<float> theSeriesYaxisMinValue = new List<float>();
			int lineSeriesCount = lineSeries.Count;
			for (int j = 0; j < lineSeriesCount; j++)
			{
				if (!activeInHierarchy(lineSeries[j])) continue;
				numSeries++;
				theSeries.Add(lineSeries[j].GetComponent<WMG_Series>());
				theSeriesCount.Add(theSeries[theSeries.Count - 1].pointValues.Count);
				theSeriesYaxisMinValue.Add(theSeries[theSeries.Count - 1].yAxis.AxisMinValue);
			}

			for (int j = 0; j < numSeries; j++)
			{
				if (maxNumValues < theSeriesCount[j]) maxNumValues = theSeriesCount[j];
				if (graphType == graphTypes.combo && theSeries[j].comboType == WMG_Series.comboTypes.bar)
				{
					numComboBarSeries++;
					if (maxNumBars < theSeriesCount[j])
					{
						maxNumBars = theSeriesCount[j];
					}
				}
			}
			...
			// For stacked charts, compute total values
			if (IsStacked)
			{
				int totalPointValuesCount = totalPointValues.Count;
				for (int i = 0; i < maxNumValues - totalPointValuesCount; i++)
				{
					totalPointValues.Add(0);
				}
				float xAxisMin = xAxis.AxisMinValue;
				bool isVertical = orientationType == WMG_Axis_Graph.orientationTypes.vertical;
				for (int i = 0; i < maxNumValues; i++)
				{
					totalPointValues[i] = 0;
					for (int j = 0; j < numSeries; j++)
					{
						if (theSeriesCount[j] > i)
						{
							totalPointValues[i] += (theSeries[j].pointValues[i].y - (isVertical ? theSeriesYaxisMinValue[j] : xAxisMin));
						}
					}
				}
			}
		...
		void UpdateBarWidth(WMG_Axis_Graph graph)
		{
			if (autoUpdateBarWidth)
			{ // Don't do anything if not auto updating bar width
				...
				barWidth = (1 - autoUpdateBarWidthSpacing) * (xAxisLengthOrienInd - maxSeriesPointCount) / numBars;
			}
		}

		void UpdateSeriesSpacing()
		{
			int lineSeriesCount = lineSeries.Count;
			for (int j = 0; j < lineSeriesCount; j++)
			{
		...
		void setAxesDualYaxisTop()
		{
			xAxis.setDualYAxesTop();
			xAxis.anchorVec = new Vector2(0.5f, 1);
			yAxis.anchorVec = new Vector2(0, 0.5f);
			yAxis2.anchorVec = new Vector2(1, 0.5f);
		}

		void setAxesDualYaxisMid()
		{
			xAxis.setDualYAxesMid();
			xAxis.anchorVec = new Vector2(0.5f, 0.5f);
			yAxis.anchorVec = new Vector2(0, 0.5f);
			yAxis2.anchorVec = new Vector2(1, 0.5f);
		}
		...
		{
			changeSpriteColor(graphBackground, color);
		}
