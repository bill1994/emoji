...
namespace Kyub.GraphMaker
{
	...
	public class WMG_Radar_Graph : WMG_Axis_Graph
	{
		...
		{
			get { return _dataSeriesLineWidth; }
			set
			{
		...
		/// <summary>
		/// Whether to do a different rendering of the grids, where lines come out from the center.
		/// </summary>
		/// <value><c>true</c> if alternate grid type; otherwise, <c>false</c>.</value>
		public bool alternateGridType
		{
			get { return _alternateGridType; }
			set
			{
				if (_alternateGridType != value)
				{
					_alternateGridType = value;
					if (_alternateGridType)
					{
						_numGrids = _numPoints;
					}
					gridsC.Changed();
				}
			}
		...
		[SerializeField] private bool _alternateGridType;
		...
		public override void Init()
		{
			...
			radarGraphC.OnChange += RadarGraphChanged;
