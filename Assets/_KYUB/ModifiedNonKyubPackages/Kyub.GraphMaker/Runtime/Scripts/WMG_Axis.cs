...
namespace Kyub.GraphMaker
{
	...
	public class WMG_Axis : WMG_GUI_Functions
	{
		...
		public enum autoGrowShrinkType { BEYOND_BY_PERCENTAGE, DATA_MIN_MAX };
		...
		/// <summary>
		/// Determines how #MinAutoGrow, #MaxAutoGrow, #MinAutoShrink, and #MaxAutoShrink work. For DATA_MIN_MAX the axes min / max are simply set directly to the min / max of the data.
		/// For BEYOND_BY_PERCENTAGE, the axes are auto growed / shrinked by the percentage WMG_Axis_Graph::autoGrowAndShrinkByPercent.
		/// </summary>
		/// <value>The type of the auto grow shrink.</value>
		public autoGrowShrinkType AutoGrowShrinkType
		{
			get { return _AutoGrowShrinkType; }
			set
			{
				if (_AutoGrowShrinkType != value)
				{
					_AutoGrowShrinkType = value;
					graphC.Changed();
					seriesC.Changed();
					graph.autoPaddingC.Changed();
				}
		...
		{
			get { return _AxisUseNonTickPercent; }
			set
			{
		...
		{
			get { return _AxisNonTickPercent; }
			set
			{
		...
		{
			get { return _HideAxisArrowTopRight; }
			set
			{
		...
		{
			get { return _HideAxisArrowBotLeft; }
			set
			{
		...
		{
			get { return _AxisTicksRightAbove; }
			set
			{
		...
		/// <summary>
		/// Whether axis ticks are on the opposite side of the axis line. When true the ticks are drawn on the inside of the graph and the ticks where the axes meet are hidden.
		/// </summary>
		/// <value><c>true</c> if opposite side ticks; otherwise, <c>false</c>.</value>
		public bool OppositeSideTicks
		{
			get { return _OppositeSideTicks; }
			set
			{
				if (_OppositeSideTicks != value)
				{
					_OppositeSideTicks = value;
					graphC.Changed();
					graph.autoPaddingC.Changed();
				}
			}
		...
		{
			get { return _AxisLabelSkipInterval; }
			set
			{
		...
		{
			get { return _AxisLabelSkipStart; }
			set
			{
		...
		{
			get { return _SetLabelsUsingMaxMin; }
			set
			{
		...
		{
			get { return _AxisLabelFontStyle; }
			set
			{
	...
	public Font AxisLabelFont { get {return _AxisLabelFont;} 
		...
		{
			get { return _numDecimalsAxisLabels; }
			set
			{
		...
		{
			get { return _AxisLabelSpaceOffset; }
			set
			{
		...
		/// <summary>
		/// Whether or not axis labels are bounded by the ends of the axes. This pivots the axis labels on the ends differently.
		/// </summary>
		/// <value><c>true</c> if axis bounded labels; otherwise, <c>false</c>.</value>
		public bool AxisBoundedLabels
		{
			get { return _AxisBoundedLabels; }
			set
			{
				if (_AxisBoundedLabels != value)
				{
					_AxisBoundedLabels = value;
					graphC.Changed();
					graph.autoPaddingC.Changed();
				}
			}
		...
		/// <summary>
		/// Whether axis labels are on the opposite side of the axis line.
		/// </summary>
		/// <value><c>true</c> if opposite side labels; otherwise, <c>false</c>.</value>
		public bool OppositeSideLabels
		{
			get { return _OppositeSideLabels; }
			set
			{
				if (_OppositeSideLabels != value)
				{
					_OppositeSideLabels = value;
					graphC.Changed();
					graph.autoPaddingC.Changed();
				}
			}
		...
		{
			get { return _AxisLabelDistBetween; }
			set
			{
		...
		public bool hideAxisLine
		{
			get { return _hideAxisLine; }
			set
			{
		...
		/// <summary>
		/// Gets or sets the axis title offset position relative to the nearest content along the axis line including labels, ticks, and the axis line itself.
		/// </summary>
		/// <value>The axis title offset.</value>
		public Vector2 AxisTitleOffset
		{
			get { return _AxisTitleOffset; }
			set
			{
		...
		[SerializeField] private autoGrowShrinkType _AutoGrowShrinkType;
		...
		[SerializeField] private bool _OppositeSideTicks;
...
#if TMP_PRESENT
		[SerializeField] private TMP_FontAsset _AxisLabelFont;
#else
	[SerializeField] private Font _AxisLabelFont; 
		...
		[SerializeField] private bool _AxisBoundedLabels;
		[SerializeField] private bool _OppositeSideLabels;
		...
		private float maxSpaceTakenByAxisLabels;
		...
		public void setDualYAxesTop()
		{
			...
			_hideTick = false;
			_AxisTick = otherAxis.AxisNumTicks - 1;
			_AxisNonTickPercent = 1;
			// left y axis
			otherAxis.setOtherArrowTopRight(false);
			otherAxis.setOtherArrowBotLeft(true);
			otherAxis.setOtherRightAbove(false);
			otherAxis.setOtherHideTick(false);
			otherAxis.setOtherAxisTick(0);
			otherAxis.setOtherAxisNonTickPercent(0);
			// right y axis
			otherAxis2.setOtherArrowTopRight(false);
			otherAxis2.setOtherArrowBotLeft(true);
			otherAxis2.setOtherRightAbove(true);
			otherAxis2.setOtherHideTick(false);
			otherAxis2.setOtherAxisTick(AxisNumTicks - 1);
			otherAxis2.setOtherAxisNonTickPercent(1);
		...
		public void setDualYAxesMid()
		{
			...
			_AxisTicksRightAbove = false;
			_hideTick = true;
			_AxisTick = otherAxis.AxisNumTicks / 2;
			_AxisNonTickPercent = 0.5f;
			// left y axis
			otherAxis.setOtherArrowTopRight(true);
			otherAxis.setOtherArrowBotLeft(true);
			otherAxis.setOtherRightAbove(false);
		...
		public void ChangeOrientation()
		{
			...
			autoGrowShrinkType tAutoGrowShrinkType = AutoGrowShrinkType;
			...
			AutoGrowShrinkType = otherAxis.AutoGrowShrinkType;
			...
			otherAxis.ChangeOrientationEnd(tLabelType, tAxisMaxValue, tAxisMinValue, tAxisNumTicks, tnumDecimalsAxisLabels, tAutoGrowShrinkType,
		...
		public void ChangeOrientationEnd(labelTypes tLabelType, float tAxisMaxValue, float tAxisMinValue, int tAxisNumTicks, int tnumDecimalsAxisLabels, autoGrowShrinkType tAutoGrowShrinkType,
										 ...
										 bool tSetLabelsUsingMaxMin, float tAxisLabelSpacing, string tAxisTitleString, List<string> tLabels, bool tHideTicks, bool tHideGrid)
		{
			...
			AutoGrowShrinkType = tAutoGrowShrinkType;
		...
		public void UpdateAxesGridsAndTicks()
		{
			...
			{ // position axis based on the percentage specified
				AxisPercentagePosition = AxisNonTickPercent;
			}
			else
			{ // position axis based on the number of ticks and the specified tick
			...
			if (!hideTicks)
			{
				...
				if (OppositeSideTicks ? AxisTicksRightAbove : !AxisTicksRightAbove)
				{
					if (isY)
					{
				...
				// Update size of ticks, and whether enabled
				List<WMG_Node> tickNodes = GetAxisTickNodes();
				for (int i = 0; i < tickNodes.Count; i++)
				{
					changeSpriteSize(tickNodes[i].objectToScale, Mathf.RoundToInt(isY ? graph.tickSize.y : graph.tickSize.x), Mathf.RoundToInt(isY ? graph.tickSize.x : graph.tickSize.y));
					SetActive(tickNodes[i].gameObject, !(OppositeSideTicks && i == otherAxis.AxisTick));
				}
			}
		...
		public void UpdateTitle()
		{
			if (AxisTitle != null)
			{
				...
				float tickOffset = 0;
				if (OppositeSideLabels == OppositeSideTicks)
				{ // don't offset label positions based on tick size if ticks drawn on the opposite side of the labels
					if (LabelType == labelTypes.ticks || (LabelType == labelTypes.groups && AxisNumTicks == graph.groups.Count))
					{
						tickOffset = graph.tickSize.y;
					}
				}
				float additionalOffset = (hideLabels ? 0 : (AxisLabelSpaceOffset + maxSpaceTakenByAxisLabels)) + (hideLabels && hideTicks ? 0 : tickOffset) + (hideLabels && hideTicks && hideAxisLine ? 0 : graph.axisWidth / 2f);
				if (isY)
				{
					if (anchorVec.x == 1)
					{ // right
						AxisTitle.transform.localEulerAngles = new Vector3(0, 0, 270);
						setAnchor(AxisTitle, anchorVec, new Vector2(0.5f, 0f), new Vector2(AxisTitleOffset.x + additionalOffset, AxisTitleOffset.y));
					}
					else
					{ // left
				...
				else
				{
					if (anchorVec.y == 1)
					{ // top
						setAnchor(AxisTitle, anchorVec, new Vector2(0.5f, 0f), new Vector2(AxisTitleOffset.x, AxisTitleOffset.y + additionalOffset));
					}
					else
					{ // bot
						setAnchor(AxisTitle, anchorVec, new Vector2(0.5f, 1f), new Vector2(AxisTitleOffset.x, -AxisTitleOffset.y - additionalOffset));
					}
				}
			}
		}
		...
		public void UpdateAxesMinMaxValues()
		{
			...
			int lineSeriesCount = graph.lineSeries.Count;
			for (int j = 0; j < lineSeriesCount; j++)
			{
				...
				int pointValuesCount = theSeries.pointValues.Count;
			...
			if (MinAutoGrow || MaxAutoGrow || MinAutoShrink || MaxAutoShrink)
			{
				...
				{
					if (AutoGrowShrinkType == autoGrowShrinkType.DATA_MIN_MAX)
					{
						AxisMaxValue = max;
					}
					else
				...
				{
					// shrink - max
					if (MaxAutoShrink && max < origMax)
					{
						AxisMaxValue = max;
					}
					// shrink - min
					if (MinAutoShrink && min > origMin)
					{
						AxisMinValue = min;
					}
				}
		...
		public void UpdateAxesLabels()
		{
			...
			if (isY)
			{
				if (OppositeSideLabels ? AxisTicksRightAbove : !AxisTicksRightAbove)
				{
			...
			for (int i = 0; i < axisLabels.Count; i++)
			{
				...
				if (LabelType == labelTypes.ticks)
				{
					...
					if (graph.IsDualY)
					{
						if (hideTick && i == otherAxis2.AxisTick) SetActive(LabelNodes[otherAxis2.AxisTick].gameObject, false);
					}
				}
				...
				bool isFirst = i == 0;
				bool isLast = i == axisLabels.Count - 1;
				// Position and pivot the labels
				if (isY)
				{
					if (OppositeSideLabels ? AxisTicksRightAbove : !AxisTicksRightAbove)
					{
						if (AxisBoundedLabels && (isFirst || isLast))
						{
							changeSpritePivot(LabelNodes[i].objectToLabel, isFirst ? WMG_Graph_Manager.WMGpivotTypes.BottomRight : WMG_Graph_Manager.WMGpivotTypes.TopRight);
						}
						else
						{
					...
					else
					{
						if (AxisBoundedLabels && (isFirst || isLast))
						{
							changeSpritePivot(LabelNodes[i].objectToLabel, isFirst ? WMG_Graph_Manager.WMGpivotTypes.BottomLeft : WMG_Graph_Manager.WMGpivotTypes.TopLeft);
						}
						else
						{
				...
				else
				{
					if (OppositeSideLabels ? AxisTicksRightAbove : !AxisTicksRightAbove)
					{
						...
						else
						{
							if (AxisBoundedLabels && (isFirst || isLast))
							{
								changeSpritePivot(LabelNodes[i].objectToLabel, isFirst ? WMG_Graph_Manager.WMGpivotTypes.TopLeft : WMG_Graph_Manager.WMGpivotTypes.TopRight);
							}
							else
							{
					...
					else
					{
						...
						else
						{
							if (AxisBoundedLabels && (isFirst || isLast))
							{
								changeSpritePivot(LabelNodes[i].objectToLabel, isFirst ? WMG_Graph_Manager.WMGpivotTypes.BottomLeft : WMG_Graph_Manager.WMGpivotTypes.BottomRight);
							}
							else
							{
				...
				if (activeInHierarchy(LabelNodes[i].gameObject))
				{
					Vector2 labelSize = getSpriteSize(LabelNodes[i].objectToLabel);
					if (isY)
					{
						maxSpaceTakenByAxisLabels = Mathf.Max(maxSpaceTakenByAxisLabels, labelSize.x * Mathf.Cos(Mathf.Abs(AxisLabelRotation) * Mathf.Deg2Rad) + labelSize.y * Mathf.Sin(Mathf.Abs(AxisLabelRotation) * Mathf.Deg2Rad) * 0.5f);
					}
					else
					{
						maxSpaceTakenByAxisLabels = Mathf.Max(maxSpaceTakenByAxisLabels, labelSize.y * Mathf.Cos(Mathf.Abs(AxisLabelRotation) * Mathf.Deg2Rad) + labelSize.x * Mathf.Sin(Mathf.Abs(AxisLabelRotation) * Mathf.Deg2Rad));
					}
				}
			}
		}
		...
		string defaultAxisLabelLabeler(WMG_Axis axis, int labelIndex)
		{
			...
			{ // for groups return the string defined at the graph level for groups, respects skip settings
			...
			{ // all other label types
				if (axis.SetLabelsUsingMaxMin)
				{ // if setting based on axis max and min values
				...
				{ // otherwise just return the label that is in the string list
