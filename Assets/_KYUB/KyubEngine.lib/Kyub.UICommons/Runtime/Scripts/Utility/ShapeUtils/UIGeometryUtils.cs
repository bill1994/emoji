...
namespace Kyub.UI.Internal
{
	public class UIGeometryUtils
	{
		...
		public class RoundingProperties
		{
			...
			[Min(2)] public int FixedResolution = 10;
			[Min(0.01f)] public float ResolutionMaxDistance = 4.0f;
			...
			)
			{
				...
				switch (overrideProperties.Resolution)
				{
					case ResolutionType.Calculated:
						float circumference = UIGeometryUtils.TwoPI * radius;
		...
		public class ShadowsProperties
		{
			...
			[Min(0.0f)] public float Distance = 0.0f;
		...
		public class ShadowProperties
		{
			...
			[Min(0.0f)] public float Size = 5.0f;
		...
		)
		{
			...
			float angleIncrement = UIGeometryUtils.TwoPI / (float)(resolution);
