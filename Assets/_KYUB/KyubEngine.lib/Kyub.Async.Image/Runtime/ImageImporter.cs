...
namespace Kyub.Async.Image
{
    ...
    const int MAX_MIPMAP_COUNT = 15;
            ...
            public ImageImporter(byte[] imageData, LoaderSettings loaderSettings) : this(Marshal.UnsafeAddrOfPinnedArrayElement(imageData, 0), imageData.Length, loaderSettings)
            {

            }

            public ImageImporter(IntPtr imageDataPtr, int imageDataPtrLen, LoaderSettings loaderSettings)
            {
                using (ConstructorMarker.Auto())
                {
                    ...
                    try
                    {
                        memoryStream = FreeImage.OpenMemory(
                          imageDataPtr,
                          (uint)imageDataPtrLen
                        );

                        if (_loaderSettings.format == FreeImage.Format.FIF_UNKNOWN)
                        {
                            _loaderSettings.format = FreeImage.GetFileTypeFromMemory(memoryStream, imageDataPtrLen);
                        }
