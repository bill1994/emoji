...
namespace MaterialUI
{
    ...
    public class MaterialRipple : MonoBehaviour, IPointerEnterHandler, IPointerDownHandler, IPointerUpHandler, IPointerExitHandler, IRippleCreator, ISelectHandler, IDeselectHandler, ISubmitHandler
    {
        ...
        /// </summary>
        [SerializeField]
        private Graphic m_ColorMatchGraphic = null;
        /// <summary>
        /// The Graphic to use to match the ripple color, if autoMatchGraphicColor is true.
        /// </summary>
        ...
        /// </summary>
        [SerializeField]
        private Graphic m_HighlightGraphic = null;
        /// <summary>
        /// The Graphic to modify when applying the highlight effect.
        ...
        private void CreateRipple(Vector2 screenPosition, Camera camera, bool oscillate = false)
        {
            if (!m_RipplesEnabled) return;

            //Try find correct Camera
            if (camera == null)
            {
                ...
                {
                    foreach (var sceneCamera in Camera.allCameras)
                    {
                        var v_isCameraFromThisObj = ((sceneCamera.cullingMask & (1 << gameObject.layer)) != 0);
                        if (v_isCameraFromThisObj)
                        {
                            camera = sceneCamera;
                            break;
                        }
