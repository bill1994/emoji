...
namespace MaterialUI
{
    ...
    public static class InspectorFields
    {
        ...
        public static bool GraphicMultiField(string label, Graphic[] graphics, Graphic visualReference = null)
        {
            ...
            if (image != null)
            {
                ImageMultiField(label, graphics.Select(graphic => graphic as Image).ToArray(), image);
                return true;
            }

            IVectorImage vectorImage = visualReference as IVectorImage;
            if (vectorImage != null)
            {
                VectorImageMultiField(label, graphics.Select(graphic => graphic as IVectorImage).ToArray(), vectorImage);
                return true;
            }
            ...
            if (text != null)
            {
                TextMultiField(label, graphics.Select(graphic => graphic as Text).ToArray(), text);
                return true;
            }

            TMPro.TMP_Text tmp_text = visualReference as TMPro.TMP_Text;
            if (tmp_text != null)
            {
                TMPTextMultiField(label, graphics.Select(graphic => graphic as TMPro.TMP_Text).ToArray(), tmp_text);
                return true;
            }
        ...
        /// <param name="disableIfMultipleObjectsSelected">Is this field drawn disabled if multiple GameObjects are selected?</param>
        /// <returns>Whether the control was successfully able to be drawn.</returns>
        public static bool TMPTextField(string label, TMPro.TMP_Text text, bool disableIfMultipleObjectsSelected = true)
        {
        ...
        /// <returns>Whether the control was successfully able to be drawn.</returns>
        public static bool TMPTextMultiField(string label, Func<GameObject, TMPro.TMP_Text> getReferenceFunc, TMPro.TMP_Text visualReference = null)
        {
            return TMPTextMultiField(label, Selection.gameObjects.Select(getReferenceFunc).ToArray());
        }
        ...
        /// <param name="visualReference">The Text to use for the 'current' values in the control if the Text values are different.</param>
        /// <returns>Whether the control was successfully able to be drawn.</returns>
        public static bool TMPTextMultiField(string label, TMPro.TMP_Text[] texts, TMPro.TMP_Text visualReference = null)
        {
        ...
        public static bool VectorImageDataField(string label, VectorImageData targetData, Object objectToUndo, Action onIconPickAction = null, bool disableIfMultipleObjectsSelected = true)
        {
            ...
            using (new DisabledScope(TooManySelected(!disableIfMultipleObjectsSelected)))
            {
                ...
                if (iconStyle == null)
                {
                    iconStyle = new GUIStyle { font = font, fontSize = font != null && font.dynamic ? vectorIconSize : 0 };
        ...
        /// <param name="disableIfMultipleObjectsSelected">Is this field drawn disabled if multiple GameObjects are selected?</param>
        /// <returns>Whether the control was successfully able to be drawn.</returns>
        public static bool VectorImageField(string label, IVectorImage vectorImage, bool disableIfMultipleObjectsSelected = true)
        {
            return VectorImageDataField(label, vectorImage.vectorImageData, vectorImage as Object, vectorImage.Refresh, disableIfMultipleObjectsSelected);
        }
        ...
        /// <returns>Whether the control was successfully able to be drawn.</returns>
        public static bool VectorImageMultiField(string label, Func<GameObject, IVectorImage> getReferenceFunc, IVectorImage visualReference = null)
        {
        ...
        /// <param name="visualReference">The VectorImage to use for the 'current' values in the control if the VectorImage values are different.</param>
        /// <returns>Whether the control was successfully able to be drawn.</returns>
        public static bool VectorImageMultiField(string label, IVectorImage[] vectorImages, IVectorImage visualReference = null)
        {
            ...
            foreach (var vectorImage in vectorImages)
            {
                Object castedVectorImage = vectorImage as Object;
                if (castedVectorImage != null)
                    objects.Add(castedVectorImage);
            }

            return VectorImageDataMultiField(label, vectorImageDatas.ToArray(), objects.ToArray(), iconPickAction, visualReference.vectorImageData);
        }
        ...
        #region ComplexFields(No GUILayout)
