...
namespace MaterialUI
{
    ...
    public class ScreenViewEditor : MaterialBaseEditor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            var materialScreen = m_ScreenView.materialScreen;

            if (materialScreen.Count > 0)
            {
                string[] names = new string[materialScreen.Count];

                var needRemoveNullScreens = false;
                for (int i = 0; i < materialScreen.Count; i++)
                {
                    names[i] = (i + 1) + " - ";
                    if (materialScreen[i] != null)
                        names[i] += materialScreen[i].name;
                    else
                        needRemoveNullScreens = true;
                }

                if (needRemoveNullScreens)
                    m_ScreenView.RemoveInvalidScreens();

                m_ScreenView.currentScreenIndex = EditorGUILayout.Popup("Current screen", m_ScreenView.currentScreenIndex, names);
            ...
            for (int i = 0; i < m_ScreenView.materialScreen.Count; i++)
            {
