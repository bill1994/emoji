...
namespace Kyub.Serialization {
    ...
    /// </summary>
    public partial class ConverterRegister {
        static ConverterRegister() {
            Converters = new List<Type>();

            foreach (var field in typeof(ConverterRegister).GetDeclaredFields()) {
                if (field.Name.StartsWith("Register_")) Converters.Add(field.FieldType);
            }

            foreach (var method in typeof(ConverterRegister).GetDeclaredMethods()) {
