    ...
    public class PrimitiveConverter : Converter {
        ...
        public override Result TryDeserialize(JsonObject storage, ref object instance, Type storageType) 
        {
            ...
            if (UseBool(storageType)) 
            {
                ...
                else if (storage.IsString && !string.IsNullOrEmpty(storage.AsString))
                {
                    long int64Value;
                    bool boolValue;

                    if (long.TryParse(storage.AsString, out int64Value))
                    {
                        sucess = Result.Success;
                        instance = ((long)Convert.ChangeType(int64Value, storageType)) != 0;
                    }
                    else if (bool.TryParse(storage.AsString, out boolValue))
                    {
                        sucess = Result.Success;
                        instance = boolValue;
                    }
            ...
            if (UseDouble(storageType) || UseInt64(storageType)) 
            {
                ...
                else if (storage.IsString)
                {
                    ...
                    if (long.TryParse(storage.AsString, out int64Value))
                    {
                        instance = Convert.ChangeType(int64Value, storageType);
                    }
                    else if (double.TryParse(storage.AsString, out doubleValue))
                    {
                        instance = Convert.ChangeType(doubleValue, storageType);
                    }
                    else
                    {
                        instance = Convert.ChangeType(0L, storageType);
                    }
                }
                else 
                {
