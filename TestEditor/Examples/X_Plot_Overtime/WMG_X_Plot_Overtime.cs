...
namespace Kyub.GraphMaker
{
	...
	public class WMG_X_Plot_Overtime : MonoBehaviour
	{
		...
		public bool displayHorizontalIndicator;
		public float blinkAnimDuration;
		float blinkScale = 2;
		public bool moveXaxisMinimum;
		public Object indicatorPrefab;
		public int indicatorNumDecimals;
		public bool moveVerticalGridLines = true;
		public bool useDataListInsteadOfRandom = false;
		public bool circularlyRepeatDataList = false; // otherwise stops once end of data list is reached
		public bool dataListIgnoreX = false; // set true to ignore x values from dataList and just evenly space the data
		public List<Vector2> dataList;
		int currentDatListIndex = 0;
		...
		float currentXaxisGridOffset = 0;
		...
		public IEnumerator plotData()
		{
			while (true)
			{
				yield return new WaitForSeconds(series1.pointValues.Count == 1 ? 0 : plotIntervalSeconds);
				if (!plottingData) break;
				animateAddPointFromEnd(new Vector2(
					useDataListInsteadOfRandom && !dataListIgnoreX ? (dataList[currentDatListIndex].x) :
					(series1.pointValues.Count == 0 ? 0 : (series1.pointValues[series1.pointValues.Count - 1].x + xInterval)),
					useDataListInsteadOfRandom ? dataList[currentDatListIndex].y : Random.Range(graph.yAxis.AxisMinValue, graph.yAxis.AxisMaxValue * 1.2f)
					), series1.pointValues.Count == 0 ? 0 : plotAnimationSeconds);
				if (useDataListInsteadOfRandom)
				{
					currentDatListIndex++;
					if (currentDatListIndex == dataList.Count)
					{
						currentDatListIndex = 0;
						if (!circularlyRepeatDataList)
						{
							plottingData = false;
						}
					}
