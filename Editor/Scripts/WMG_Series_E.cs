...
namespace Kyub.GraphMaker
{
	...
	public class WMG_Series_E : WMG_E_Util
	{
...
#if TMP_PRESENT
			series.dataLabelsFont = (TMP_FontAsset)EditorGUILayout.ObjectField("Data Label Font", series.dataLabelsFont, typeof(TMP_FontAsset), false);
#else
		...
		void DrawShading()
		{
			...
			ExposeProperty(fields["linesUseComputeShader"]);
			series.neverCreateLines = EditorGUILayout.Toggle("Never Create Lines", series.neverCreateLines);
			series.neverCreatePoints = EditorGUILayout.Toggle("Never Create Points", series.neverCreatePoints);
			series.multiSeriesEnabled = EditorGUILayout.Toggle("Multi Series Enabled", series.multiSeriesEnabled);
			series.multiSeriesNumSeries = EditorGUILayout.IntField("Multi Series Num Series", series.multiSeriesNumSeries);
		}
